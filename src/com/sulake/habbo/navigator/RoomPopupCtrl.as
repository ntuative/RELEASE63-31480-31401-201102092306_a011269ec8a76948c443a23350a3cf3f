package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2335 = new RoomThumbnailRenderer(param1);
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_172 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_172.visible = true;
         Util.hideChildren(this.var_172);
         this.refreshRoomName(this.var_172,this.var_112);
         if(this.var_112.ownerName != "" && this.var_112.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_172,!!this.var_112.event ? this.getEventCtg(this.var_112.categoryId) : this.getRoomCtg(this.var_112.categoryId));
         this.refreshRoomDesc(this.var_172,this.var_112);
         this.refreshExtraCont();
         this.refreshInfo(this.var_172,"trading_allowed",this.var_112.allowTrading);
         this.refreshInfo(this.var_172,"doormode_doorbell",this.var_112.doorMode == 1);
         this.refreshInfo(this.var_172,"doormode_password",this.var_112.doorMode == 2);
         Util.moveChildrenToColumn(this.var_172,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_172.height = Util.getLowestPoint(this.var_172);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_172,this.var_112.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_172.findChildByName("roomowner"));
         if(this.var_112.ownerName != "" && this.var_112.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_172.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1075.refreshTags(_loc1_,this.var_112.tags);
         ...
         this.var_2335.refreshThumbnail(_loc1_,this.var_112.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_230,Tabs.const_264,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
