package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_696:ColorTransform;
      
      private var var_1316:ColorTransform;
      
      private var var_1631:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_771:Number = 1.0;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_696 = new ColorTransform();
         this.var_696.alphaMultiplier = 1;
         this.var_1316 = new ColorTransform();
         this.var_1631 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_92 = this.var_1063[param1];
         this.var_858 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1630 >= 0)
         {
            this.var_240 = this._visualization.getSprite(this.var_1630);
            if(this.var_240 && this.var_240.asset)
            {
               ...
               if(this.var_83 == null)
               {
                  ...
                  if(this.var_696.alphaMultiplier != 1)
                  {
                     this.var_579 = new BitmapData(this.var_83.width,this.var_83.height,true,4278190080);
                  }
               }
               this.var_2244 = -this.var_240.offsetX;
               this.var_2245 = -this.var_240.offsetY;
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_92.y * this.var_1062;
         }
         return 0;
      }
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_858 && this.var_92.hasIgnited)
         {
            this.var_858 = true;
         }
         _loc8_ = this.var_1158 * this.var_1062;
         this.var_92.update();
         if(this.var_858)
         {
            if(this.var_92.roomObjectSpriteId >= 0)
            ...
            {
               this.var_83.draw(this.var_579,this.var_1631,this.var_696,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_92.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2244 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1062;
               _loc4_ = this.var_2245 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1062;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1316.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_83.draw(_loc7_,this._translationMatrix,this.var_1316,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2246 = parseInt(param1.@size);
         this.var_1630 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1158 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1062 = this.var_2246 / 64;
         this.var_771 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_771 = Math.min(this.var_771,1);
         this.var_696.alphaMultiplier = this.var_771;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc19_ = _loc23_.@is_emitter == "INotify" ? false : true;
