package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1816 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1167 = this.var_1816.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1167)
         {
            this.var_1167.procedure = this.onButtonClick;
         }
         this.var_1168 = this.var_1816.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1168)
         {
            this.var_1168.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_256 >= this._localizations.length - 1)
         {
            this.var_1168.disable();
         }
         else
         {
            this.var_1168.enable();
         }
         if(this.var_256 <= 0)
         {
      ...
      {
         ++this.var_256;
         this.var_256 = Math.min(this.var_256,this._localizations.length - 1);
         this.openPage();
      }
      ...
      private function localize() : void
      {
         ...
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this._localizations[this.var_256][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this._localizations[this.var_256][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this._localizations[this.var_256][1]).value;
