package com.sulake.habbo.tracking
{
   import com.sulake.habbo.configuration.IHabboConfigurationManager;
   
   public class ToolbarClickTracker
   {
       
      
      private var var_1238:IHabboTracking;
      
      private var var_1811:Boolean = false;
      
      private var var_2302:int = 0;
      
      private var var_1654:int = 0;
      
      public function ToolbarClickTracker(param1:IHabboTracking)
      {
         super();
         this.var_1238 = param1;
      }
      
      public function dispose() : void
      {
         this.var_1238 = null;
      }
      
      public function configure(param1:IHabboConfigurationManager) : void
      {
         this.var_1811 = Boolean(parseInt(param1.getKey("toolbar.tracking.enabled","1")));
         this.var_2302 = parseInt(param1.getKey("toolbar.tracking.max.events","100"));
      }
      
      public function track(param1:String) : void
      {
         if(!this.var_1811)
         {
            return;
         }
         ++this.var_1654;
         if(this.var_1654 <= this.var_2302)
         {
            this.var_1238.track("toolbar",param1);
         }
      }
   }
}
