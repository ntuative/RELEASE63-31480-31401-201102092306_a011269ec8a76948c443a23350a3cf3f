package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_402 = this.getPetTypeIndexFromProduct(_loc5_.localizationId);
         if(this.var_402 >= 8)
         {
            return false;
         }
         ...
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc9_ = _loc8_.avatarRenderManager.petDataManager.getPetData(this.var_402,this);
               if(_loc9_)
               {
                  this.var_208 = _loc9_.sellableBreeds;
                  this.var_360 = _loc9_.colors;
         ...
         var _loc6_:int = 0;
         while(_loc6_ < this.var_208.length)
         {
            _loc10_ = this.var_208[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(this.getRaceLocalizationKey(this.var_402,_loc10_.id),this.getRaceLocalizationKey(this.var_402,_loc10_.id)));
            _loc6_++;
         }
         this._offers.add(this.var_402,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_664,this.onApproveNameResult);
      ...
      public function onDropMenuEvent(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowEvent.const_59)
         {
            ...
            if(_loc3_ >= this.var_208.length)
            {
               return;
            }
            this.var_1843 = _loc3_;
            this.updateImage();
         }
      }
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(this.var_499 < 0 || this.var_499 > this.var_360.length)
         {
            this.var_499 = 0;
         }
         this.updateImage();
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_499 >= 0 && this.var_499 < this.var_360.length)
         {
            _loc7_ = this.var_360[this.var_499] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = this.var_208[this.var_1843] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_402,_loc3_.id,_loc2_,AvatarScaleType.const_53,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_314,AvatarAction.const_889);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_499 >= this.var_360.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_360[this.var_499] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = this.var_208[this.var_1843] as IBreed;
